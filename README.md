# Python Fundamentals - Learning Repository

A comprehensive collection of Jupyter notebooks and Python scripts covering fundamental programming concepts. This repository serves as a structured learning path for Python beginners and intermediate learners.

## 📚 Course Content

### Core Python Concepts
- **[1.0 Basic Concepts](./1.0-Basic.ipynb)** - Introduction to Python syntax and basic programming concepts
- **[2.0 Data Types](./2.0-Data%20Types.ipynb)** - Understanding Python's built-in data types
- **[3.0 Conditional Statements](./3.0-ConditionalStatements.ipynb)** - Control flow with if/elif/else statements

### Data Structures
- **[4. Lists](./4-DS_List.ipynb)** - Working with Python lists and list operations
- **[5. Sets](./5-Sets.ipynb)** - Understanding sets and set operations
- **[6. Dictionaries](./6.0%20Dictinaries.ipynb)** - Key-value pairs and dictionary methods
- **[7. Tuples](./7.0%20Tuple.ipynb)** - Immutable sequences and tuple operations

### Functions & Advanced Concepts
- **[8. Functions](./8%20Function.ipynb)** - Function definition, parameters, and return values
- **[9. Function Examples](./9%20functions%20examples.ipynb)** - Practical function implementations and use cases
- **[10. Lambda Functions](./10%20Lambda%20Function.ipynb)** - Anonymous functions and functional programming
- **[11. MAP Function](./11%20MAP%20function.ipynb)** - Higher-order functions and data transformation

## 🚀 Getting Started

### Prerequisites
- Python 3.11 or higher
- Jupyter Notebook or JupyterLab

### Installation

1. **Clone the repository:**
   ```bash
   git clone <your-repo-url>
   cd python-fundamentals
   ```

2. **Install dependencies:**
   ```bash
   pip install -r requirements.txt
   ```
   
   Or using uv (recommended):
   ```bash
   uv sync
   ```

3. **Launch Jupyter Notebook:**
   ```bash
   jupyter notebook
   ```

### Quick Start
Run the main Python script to verify your setup:
```bash
python main.py
```

## 📖 Learning Path

For optimal learning experience, follow this recommended sequence:

1. **Basics** → [Basic Concepts](./1.0-Basic.ipynb)
2. **Data Types** → [Data Types](./2.0-Data%20Types.ipynb)
3. **Control Flow** → [Conditional Statements](./3.0-ConditionalStatements.ipynb)
4. **Data Structures** → [Lists](./4-DS_List.ipynb) → [Sets](./5-Sets.ipynb) → [Dictionaries](./6.0%20Dictinaries.ipynb) → [Tuples](./7.0%20Tuple.ipynb)
5. **Functions** → [Functions](./8%20Function.ipynb) → [Function Examples](./9%20functions%20examples.ipynb)
6. **Advanced** → [Lambda Functions](./10%20Lambda%20Function.ipynb) → [MAP Function](./11%20MAP%20function.ipynb)

## 🛠 Project Structure

```
python-fundamentals/
├── 1.0-Basic.ipynb                 # Python basics and syntax
├── 2.0-Data Types.ipynb           # Data types overview
├── 3.0-ConditionalStatements.ipynb # Control flow
├── 4-DS_List.ipynb                # List data structure
├── 5-Sets.ipynb                   # Set operations
├── 6.0 Dictinaries.ipynb          # Dictionary methods
├── 7.0 Tuple.ipynb                # Tuple operations
├── 8 Function.ipynb               # Function fundamentals
├── 9 functions examples.ipynb     # Practical examples
├── 10 Lambda Function.ipynb       # Anonymous functions
├── 11 MAP function.ipynb          # Higher-order functions
├── main.py                        # Main Python script
├── sample.txt                     # Sample data file
├── pyproject.toml                 # Project configuration
└── README.md                      # This file
```

## 🎯 Learning Objectives

By completing this course, you will:
- ✅ Master Python syntax and basic programming concepts
- ✅ Understand and work with all major Python data types
- ✅ Implement control flow using conditional statements
- ✅ Manipulate data using lists, sets, dictionaries, and tuples
- ✅ Create and use functions effectively
- ✅ Apply functional programming concepts with lambda and map functions

## 📋 Dependencies

This project uses the following packages:
- **ipykernel** (≥6.29.5) - Jupyter kernel for Python
- **numpy** (≥2.3.1) - Numerical computing library
- **pandas** (≥2.3.0) - Data manipulation and analysis

## 🤝 Contributing

Contributions are welcome! Please feel free to:
- Report bugs or issues
- Suggest improvements to existing notebooks
- Add new examples or exercises
- Improve documentation

## 📄 License

This project is open source and available under the [MIT License](LICENSE).

## 🔗 Additional Resources

- [Official Python Documentation](https://docs.python.org/3/)
- [Jupyter Notebook Documentation](https://jupyter-notebook.readthedocs.io/)
- [Python Tutorial for Beginners](https://www.python.org/about/gettingstarted/)

---

**Happy Learning! 🐍✨**

> This repository is part of a comprehensive Python learning journey. Each notebook is designed to build upon previous concepts while introducing new programming paradigms.